...
class ToolManager:
    ...
    async def execute_tool(self, tool_name: str, query: str) -> dict:
        ...
        if tool_name == "memory":
            if self.memory_col is None: return {"success": False, "source": tool_name, "content": "", "confidence": 0.0, "metadata": {}}
            return await tool.execute(query, self.memory_col)
        elif tool_name == "documents":
            if self.docs_col is None: return {"success": False, "source": tool_name, "content": "", "confidence": 0.0, "metadata": {}}
            return await tool.execute(query, self.docs_col)
        elif tool_name == "web":
            if self.tavily is None: return {"success": False, "source": tool_name, "content": "", "confidence": 0.0, "metadata": {}}
