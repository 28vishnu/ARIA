...
class PersonalityEngine:
    ...
    async def apply_personality(
        ...
        try:
            ...
            # Apply conversational styling only to genuinely conversational
            # responses. Tool/routed responses must preserve their authoritative
            # structured content.
            if source not in {
            ...
            if source == ResponseSource.WEATHER:
                logger.info(
                    "[Personality] Weather response protected from all rewriting."
                )
            ...
            if source == ResponseSource.WEATHER:
                logger.info(
                    "[Personality] Weather response protected from LLM rewriting."
                )
                return self._post_process(reply)
